...
export default function InsightsPanel({ captured, rejected, prefs }) {
  ...
  const matchRate =
    captured.length + rejected.length > 0
      ? Math.round((captured.length / (captured.length + rejected.length)) * 100)
      : 0;
  ...
  const bySpec = SPECS.map((specialty) => ({
    ...
    total: captured
      .filter((item) => item.spec === specialty)
  ...
  const insights = [
    ...
    prefs.days.length < 4
      ? {
          tone: C.warning,
          text: `Disponibilidade atual em ${prefs.days.length} dias. Mais dias aumentam chances de captura.`,
        }
      : null,
  ].filter(Boolean);
  ...
  return (
    <Card>
      <div style={{ marginBottom: 9, fontSize: 12, color: C.text2, fontWeight: 700 }}>
      ...
      {bySpec.length > 0 ? (
        <div style={{ marginTop: 14 }}>
          <div style={{ fontSize: 12, color: C.text2, marginBottom: 8, fontWeight: 700 }}>
            Ganhos por especialidade
          </div>
          {bySpec.map((item, index) => {
            const percentage =
              bySpec[0].total > 0 ? Math.round((item.total / bySpec[0].total) * 100) : 0;
            return (
